...
class Category extends MY_Controller
{
    ...
    {
        // Cek apakah ada id yang ada ditabel category
        $data['content']    = $this->category->where('id', $id)->first();

        // Jika tidak ada akan muncul warning
        // Akan dikembalikan ke halaman index
        if (!$data['content']) {
            $this->session->set_flashdata('warning', 'Maaf data tidak ditemukan!');
            redirect(base_url('category'));
        }

        // Jika datanya ada akan dilanjutkan
        // Apakah ketika melakukan proses edit ini menggunakan method post atau tidak
        if (!$_POST) {
